...
const ProductCompatibleRule: ProductRule = {
  ...
  validate(build: BuildPartDetails) {
    ...
    if (!build.ram || build.ram.length === 0) result[Products.RAM] = "RAM is required.";

    if ((!build.ssd || build.ssd.length === 0) && (!build.hdd || build.hdd.length === 0)) {
      result[Products.SSD] = "At least one storage device (SSD or HDD) is required.";
      result[Products.HDD] = "At least one storage device (SSD or HDD) is required.";
    }
    ...
    const coolerCount =
      Number(Boolean(build.cooler)) + Number(Boolean(build.aio)) + Number(Boolean(build.cpu_block));
    ...
    if (
      ...
      if (!build.cpu_block) result[Products.CPU_BLOCK] = "CPU Block is required.";
    }
