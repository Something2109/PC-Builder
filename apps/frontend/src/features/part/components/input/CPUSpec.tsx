...
export const Components: InfoComponentObject<CPUSpec.DTO> = {
  ...
  total_cores: (field) => (
    <SuffixInput suffix="Cores" type="number" {...mapChange(field, "number")} />
  ),
  total_threads: (field) => (
    <SuffixInput suffix="Threads" type="number" {...mapChange(field, "number")} />
  ),

  lithography: (field) => <Input {...mapChange(field, "string")} />,
};
