...
export const DTOVisualizerVisual = ({
  ...
  const activeBasicFields = onlyErrors
    ? basicFields.filter((key) => errors && getErrorAtPath(errors, key))
    : basicFields;

  const basicInfo: [string, unknown][] = activeBasicFields.map((key) => [key, data[key]]);
  ...
  const activeDetailedKeys = onlyErrors
    ? Array.from(detailedInfoKeys).filter((key) => errors && hasErrorInSubtree(errors, key))
    : Array.from(detailedInfoKeys);

  const detailedInfo: [string, unknown][] = activeDetailedKeys.map((key) => [
    key,
    data[key],
  ]);
              ...
              return (
                ...
                >
                  <div className="flex justify-between items-center gap-2">
                    ...
                    {source && (
                      <span
                        className="text-[9px] bg-slate-900 text-slate-400 border border-slate-800 px-1.5 py-0.5 rounded font-mono truncate max-w-37.5"
          ...
          >
            ...
            {Array.isArray(val) ? (
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        {(() => {
                          ...
                          const activeSubKeys = onlyErrors
                            ? Array.from(subKeys).filter((subKey) => errors && getErrorAtPath(errors, `${key}.${idx}.${subKey}`))
                            : Array.from(subKeys);

                          return activeSubKeys.map((subKey) => {
                            ...
                            return (
                              ...
                              >
                                <div className="flex justify-between items-center gap-2">
                                  ...
                                  {source && (
                                    <span
                                      className="text-[8px] bg-slate-950 text-slate-500 border border-slate-855 px-1 py-0.2 rounded font-mono truncate max-w-30"
                                      title={`Mapped from: "${source.rawKey}" (${source.matchType} match)`}
                                    >
                                      from: &quot;{source.rawKey}&quot;
                                    </span>
              ...
              (() => {
                ...
                const activeSubKeys = onlyErrors
                  ? Array.from(subKeys).filter((subKey) => errors && getErrorAtPath(errors, `${key}.${subKey}`))
                  : Array.from(subKeys);

                return (
                  <div className="bg-slate-955/80 border border-slate-800/50 p-3 rounded-lg grid grid-cols-1 sm:grid-cols-2 gap-3">
