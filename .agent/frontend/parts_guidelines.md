## Frontend Guideline: PC Part Handling (Next.js App Router)
...
- `/part/[part]/[id]` — detail view (read-only table).
- `/part/[part]/[id]/edit` — edit view (ADMIN).
- `/part/[part]/new` — create new part (ADMIN).
...
- Create/update/delete guarded in UI via `AuthRole` (client) and `ServerAuthRole` (server) with `roles=[ADMIN]`.
- On failed API calls, surface message via `NotificationBar` in forms; on list/detail pages, route to Not Found.

### URL & State Conventions

- Maintain a single source of truth for filters in `URLSearchParams`.
- When submitting filters, use `form` submit to preserve SSR and pagination behavior.
- Use `router.replace()` for resets to clear all filters.
...
- Navigate from `/part` to `/part/:product` verifying query propagation between list, filters, and pagination.
- Create a part then confirm redirect to edit page and presence of info blocks.
- Toggle filter attributes and confirm options reflect server-calculated values for the current context.
